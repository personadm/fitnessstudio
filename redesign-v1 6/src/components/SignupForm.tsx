...
function HighlightCard({ highlight }: { highlight: TopHighlight }) {
  ...
  // Default: zart-Petrol hinterlegt. Gold: warme Gold-Hinterlegung als
  // visueller "Spezial-Spezial"-Marker (z.B. für Krankenkassen-Erstattung).
  const bg = isGold ? GOLD_BG : "rgba(15,110,86,0.08)";
  const borderColor = isGold ? "rgba(133,79,11,0.3)" : "rgba(15,110,86,0.2)";
  const iconBg = isGold ? "rgba(133,79,11,0.18)" : "rgba(15,110,86,0.18)";
  const iconColor = isGold ? GOLD_TEXT : PETROL;
  const textColor = isGold ? GOLD_TEXT : "#2C2C2A";
  const subtitleColor = isGold ? "rgba(133,79,11,0.85)" : "#5F5E5A";
  ...
  return (
    ...
    >
      {/* Stern als visuelles Spezial-Spezial-Signal bei Gold */}
      {isGold && (
        <span
          className="absolute right-3 top-3 inline-flex h-5 w-5 items-center justify-center rounded-full text-xs"
          style={{ backgroundColor: GOLD_TEXT, color: GOLD_BG }}
          aria-hidden
